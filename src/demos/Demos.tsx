...
import ProfileSass from './layout/ProfileSass'
...
const routes = [
  ...
  { path: 'layout-profile-sass', component: ProfileSass },
    ...
    <div className={ss.container}>
      {width > threshold && (
        <ul className={ss.menuList}>
          {routes.map((rou) => (
            <li key={rou.path} className={ss.menuItem}>
              <span
                onClick={() => handleClick(rou.path)}
                className={`
                  ${ss.menuLink} 
                  ${getCurHash() === rou.path && ss.menuLinkActive}`}
              >
                {rou.path}
              </span>
            </li>
      ...
      <div
        className={`${ss.component} 
          ${width <= threshold && ss.componentMobile}`}
      >
        {resolveCur()}
      </div>
    </div>
